...
async def get_multimodal_response(user_id: int, user_input: str) -> str:
    ...
    # === ДОБАВЛЯЕМ ДАННЫЕ ОПРОСА И ПЕРСОНАЛИЗАЦИЮ ===
    user = get_user(user_id)
    poll_data = user.poll_data

    if poll_data and any(poll_data.values()):
        # Добавляем данные опроса в системный промпт
        system_prompt += (
        ...
        # === ДОБАВЛЯЕМ ПРАВИЛО ПЕРСОНАЛИЗАЦИИ ТОЛЬКО ЕСЛИ ЕСТЬ ИМЯ ===
        if poll_data.get("name"):
            system_prompt += (
                "\n\n=== ВАЖНОЕ ПРАВИЛО ПЕРСОНАЛИЗАЦИИ ===\n"
                "ВСЕГДА обращайся к пользователю по имени в КАЖДОМ ответе.\n"
                "Даже в коротких ответах типа 'спасибо', 'пожалуйста' — используй имя.\n"
                "=========================================\n"
            )
